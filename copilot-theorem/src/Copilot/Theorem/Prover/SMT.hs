...
getVars = nubBy' (compare `on` varName) . concatMap getVars'
  where
    getVars' :: Expr -> [VarDescr]
    getVars' = \expression -> case expression of
      ConstB _             -> []
      ConstI _ _           -> []
      ConstR _             -> []
...
kInduction' startK maxK s as ps = (fromMaybe (Output P.Unknown ["proof by k-induction failed"]) . fst)
  ...
  where
    induction k = do
      ...
      entailment Base (modelInit ++ base) baseInv >>= \result -> case result of
        Sat     -> invalid $ "base case failed for " ++ proofKind k
        Unknown -> unknown
        Unsat   ->
          if not inductive then valid ("proved without induction")
          else entailment Step step stepInv >>= \result -> case result of
            Sat     -> unknown
            Unknown -> unknown
  ...
  where
    script  = do
      ...
      if inductive
        ...
        else entailment Base (modelInit ++ base) (map (Op1 Bool Not) baseInv) >>= \result -> case result of
...
onlyValidity' s as ps = (fromJust . fst) <$> runPS (script <* stopSolvers) s
  where
    script  = do
      ...
      if inductive
        then unknown' "proposition requires induction to prove."
        else entailment Base (modelInit ++ base) baseInv >>= \result -> case result of
