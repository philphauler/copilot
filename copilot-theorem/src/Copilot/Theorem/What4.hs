...
proveInternal solver spec k = do
  ...
  let proveProperties = forM (CS.specProperties spec) $ \pr -> do
        ...
        satRes <-
          case solver of
            CVC4 -> liftIO $ WS.runCVC4InOverride sym WS.defaultLogData clauses $ \result -> case result of
              WS.Sat (ge, _) -> k' (WS.Sat ge)
              WS.Unsat x -> k' (WS.Unsat x)
              WS.Unknown -> k' WS.Unknown
            DReal -> liftIO $ WS.runDRealInOverride sym WS.defaultLogData clauses $ \result -> case result of
              WS.Sat (c, m) -> do
                ge <- WS.getAvgBindings c m
                k' (WS.Sat ge)
              WS.Unsat x -> k' (WS.Unsat x)
              WS.Unknown -> k' WS.Unknown
            Yices -> liftIO $ WS.runYicesInOverride sym WS.defaultLogData clauses $ \result -> case result of
              WS.Sat ge -> k' (WS.Sat ge)
              WS.Unsat x -> k' (WS.Unsat x)
              WS.Unknown -> k' WS.Unknown
            Z3 -> liftIO $ WS.runZ3InOverride sym WS.defaultLogData clauses $ \result -> case result of
              WS.Sat (ge, _) -> k' (WS.Sat ge)
              WS.Unsat x -> k' (WS.Unsat x)
              WS.Unknown -> k' WS.Unknown
